...
| Supabase Auth | Identidad y sesión únicamente del personal autorizado por un administrador. |
...
Equipo inicial: dos agendas de peluquero y dos accesos administrativos; una cuenta puede administrar y atender. `user_roles` conserva un único rol principal, mientras `professionals` determina si la cuenta tiene agenda. Un rol `admin` vinculado a un profesional activo conserva disponibilidad. Los peluqueros sin rol administrativo solo gestionan su agenda. El alta desde `/api/staff/users` verifica el token y rol activo del administrador; `provision_staff` solo admite ejecución por `service_role` y registra auditoría. El servidor no modifica una cuenta ya registrada al recibir otra invitación.
