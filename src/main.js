...
import { money, escapeHtml as e, dateKey, timeLabel, longDate, shiftDate, periodRange, statusLabels, paymentLabels, mayCancel, icsEvent } from './lib/domain.js';
...
document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (button.dataset.calendar) return showModal('Tu cita en el calendario', `<p class="notice">La sincronización automática con Google Calendar todavía requiere implementar y configurar OAuth. Esta descarga es solo una copia y no se actualiza si cambias o cancelas la reserva.</p><button class="outline-button full-width mt-5" data-download-ics="${button.dataset.calendar}">Descargar evento (.ics)</button>`);
