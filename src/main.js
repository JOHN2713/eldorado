...
import { configured, configurationError, supabase, bootstrap, identity, rpc, friendly, signIn, settingsWithSetup, staffUsers, createStaff } from './services/api.js';
...
const state = { data: null, me: null, path: location.pathname, booking: { step: 1, date: dateKey(), professional: '', service: '', slot: null }, slots: [], appointments: [], sales: [], breaks: [], notifications: [], staff: [], authMode: authModeFromLocationHash(location.hash), date: dateKey(), period: 'week', from: periodRange('week')[0], to: periodRange('week')[1] };
...
function panel(content) {
  const links = [['agenda', 'calendar', 'Agenda'], ['ventas', 'chart', 'Ventas'], ['descansos', 'coffee', 'Descansos'], ['usuarios', 'user', 'Usuarios'], ['configuracion', 'settings', 'Configuración']].filter(([p]) => !configured || isAdmin() || ['agenda', 'descansos'].includes(p));
...
function usersPage() {
  const serviceChecks = state.data.services.map((service) => `<label class="checkbox compact"><input type="checkbox" name="service" value="${service.id}" checked><span>${e(service.name)}</span></label>`).join('');
  const rows = state.staff.map((item) => `<tr><td><strong>${e(item.email || 'Correo no disponible')}</strong>${item.professional ? `<div class="caption">Agenda: ${e(item.professional.name)}</div>` : ''}</td><td>${item.role === 'admin' ? 'Administrador' : 'Peluquero'}</td><td><span class="badge ${item.active ? 'completed' : 'cancelled'}">${item.active ? 'Activo' : 'Sin acceso'}</span>${item.professional ? ` <span class="badge ${item.professional.active ? 'completed' : ''}">${item.professional.active ? 'Agenda activa' : 'Agenda por configurar'}</span>` : ''}</td></tr>`).join('');
  return `<div class="panel-heading"><div><div class="eyebrow mb-3">ACCESOS DEL NEGOCIO</div><h1>Administración de usuarios</h1></div></div><div class="user-management"><section class="surface"><h2 class="mb-5">Invitar un usuario</h2><form id="staff-form">${field('Nombre', 'name', 'text', '', 'required minlength="2" maxlength="100" autocomplete="name"')}${field('Correo electrónico', 'email', 'email', '', 'required maxlength="254" autocomplete="email"')}${select('Tipo de acceso', 'role', [['barber','Peluquero'],['admin','Administrador']], 'barber')}<label class="checkbox"><input type="checkbox" name="professional" checked disabled><span>Crear también una agenda de peluquero. Para el rol Peluquero siempre se crea.</span></label><fieldset class="service-picker"><legend>Servicios que podrá atender</legend>${serviceChecks}</fieldset><div class="notice">El usuario recibirá un correo para definir su contraseña. Una agenda nueva queda inactiva hasta guardar sus jornadas en Configuración.</div><div class="form-error" role="alert" tabindex="-1"></div><button class="button full-width mt-5" type="submit" ${!configured ? 'disabled' : ''}>Enviar invitación ${icon('arrow')}</button></form></section><section class="surface"><h2 class="mb-5">Usuarios registrados</h2>${rows ? `<div class="table-wrap"><table><thead><tr><th>Cuenta</th><th>Rol principal</th><th>Estado</th></tr></thead><tbody>${rows}</tbody></table></div>` : empty('No hay usuarios del equipo registrados.', 'user')}<p class="caption mt-5">Desactivar o cambiar el rol de una cuenta existente requiere una acción administrativa controlada; esta pantalla no borra usuarios ni historial.</p></section></div>`;
}
function bookingSetupSummary() {
  ...
  const readyCount = setup ? activeProfessionals.filter((p) => !professionalSetupIssues(p, setup.roles, setup.assignments).length).length : null;
...
async function render() {
  ...
  try {
    ...
    if (isPanel && configured && ['ventas','usuarios','configuracion'].some((x) => state.path.endsWith(x)) && !isAdmin()) { root.innerHTML = shell(empty('Esta sección está disponible solo para el administrador.', 'shield')); return; }
    ...
    } else if (state.path === '/panel/usuarios') {
      state.staff = configured ? await staffUsers() : [];
      content = panel(usersPage());
...
document.addEventListener('change', (event) => {
  if (event.target.form?.id === 'staff-form' && ['role', 'professional'].includes(event.target.name)) {
    const form = event.target.form, barber = form.elements.role.value === 'barber';
    if (event.target.name === 'role') form.elements.professional.checked = barber;
    form.elements.professional.disabled = barber;
    form.querySelector('.service-picker').disabled = !barber && !form.elements.professional.checked;
  }
...
document.addEventListener('submit', async (event) => {
  ...
  try {
    if (form.id === 'slots-form') { state.booking.date = value('date'); state.booking.professional = value('professional'); await loadSlots(); await render(); }
      ...
      const result = await createStaff({ email: value('email'), name: value('name'), role, professional: role === 'barber' || f.has('professional'), serviceIds: f.getAll('service') });
      state.staff = await staffUsers();
      form.reset();
      toast(result.invited ? 'Invitación enviada. La persona debe abrir el correo y definir su contraseña.' : 'La cuenta existente quedó autorizada. Puede ingresar o recuperar su contraseña.', 'Usuario creado');
      await render();
    }
